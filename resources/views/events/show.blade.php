...
<section style="padding: 80px 0; background: var(--bg-light);">
    <div class="container" style="max-width: 900px;">
        @if($event->image)
            <div style="border-radius: 20px; overflow: hidden; margin-bottom: 40px; border: 2px solid var(--brand-gold); box-shadow: var(--shadow-hover);">
                <img src="{{ asset('uploads/' . $event->image) }}" alt="{{ $event->title }}" style="width: 100%; max-height: 480px; object-fit: cover;">
            </div>
        @endif

        <div style="background: #ffffff; padding: 45px; border-radius: 20px; border: 1px solid var(--border-light); box-shadow: var(--shadow-soft);">
            ...
            @if($event->link)
                <div style="margin-top: 20px;">
                    <a href="{{ $event->link }}" target="_blank" class="btn-pill-gold">Visit External Link <i class="fa-solid fa-up-right-from-square"></i></a>
                </div>
            @endif
        </div>
    </div>
